# Contributing to RABERU

Thank you for your interest in RABERU. Bug reports, documentation fixes, a missing helper and whole
new parts of the keyword protocol are all welcome.
...
ask on [our Discord](https://discord.gg/rfsYYexss). No question is too small, and a five minute
...
merged pull requests. **You are welcome.** A student who wants to see how named parameters are built
in C++ is exactly who we wrote this for, and nobody is going to hold a deadline against you.
...
So say so. In the issue, or on [Discord](https://discord.gg/rfsYYexss): "I have two weeks and I
...
issue](https://github.com/jfalcou/raberu/labels/good%20first%20issue) label lists work that is
...
```bash
cmake --preset gcc
cmake --build build/gcc --target unit.keywords.exe
ctest --preset gcc
```

One target per test file, and CMake globs the test directories, so a new `.cpp` needs no CMake edit.
RABERU is small: the whole suite builds in seconds, so run it rather than a single target.
...
- **the documentation of what you touched**, with the `raberu-doxygen` target still silent, since a
...
and the call that shows it. A Godbolt link that reproduces it replaces all three.

**Work on an open issue.** The [open issues](https://github.com/jfalcou/raberu/issues) are all fair
...
[Discord](https://discord.gg/rfsYYexss) before you write it: a design that has been talked through
...
cmake --build build/gcc --target raberu-doxygen
```

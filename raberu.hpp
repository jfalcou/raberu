...
namespace rbr::_
{
  ...
  template<typename K, typename Ks> struct append_if : append_if_impl<K, Ks, contains<K, Ks>::value>
  {
  };
  template<typename K1, typename K2> struct uniques;
  template<typename K1s, typename K2, typename... K2s>
  struct uniques<K1s, keys<K2, K2s...>> : uniques<typename append_if<K2, K1s>::type, keys<K2s...>>
  {
  };
  template<typename K1s> struct uniques<K1s, keys<>>
  {
    using type = K1s;
  };
...
namespace rbr::concepts
{
  ...
  template<typename S>
  concept settings = requires(S const& s) { typename S::rbr_settings; };
...
namespace rbr
{
  namespace _
  {
    template<typename K, concepts::keyword... Kept> struct filter
    {
    ...
    struct select_keys<K, rbr::settings<Os...>> : decltype((filter<typename K::tag_type>{} + ... + _::keys<Os>{}))
    {
    };
  }
  ...
  namespace result
  {
    template<concepts::keyword K, concepts::option... O> struct drop
    {
      using type = decltype(rbr::drop(std::declval<K>(), std::declval<O>()...));
    };
    template<concepts::keyword K, concepts::option... O> using drop_t = typename drop<K, O...>::type;
  }
}
namespace rbr
{

...
TTS_CASE("Check settings(...) get_or_eval behavior - named parameters")
{
  auto values = rbr::settings(value_ = 1337.42f);
  auto or_else = []<typename T>(rbr::type_<T>) { return sizeof(T); };

  TTS_EQUAL(get_or_eval<char>  (values, or_else)  , 1ULL    );
  TTS_EQUAL(get_or_eval<double> (values, or_else) , 8ULL    );
  TTS_EQUAL(get_or_eval<float>(values, or_else)   , 1337.42f);
}
...
TTS_CASE("Check settings(...) get_or_eval constexpr behavior - simple parameters")
{
  ...
  TTS_EXPECT( bool_<get_or_eval<double> (values, or_else) == 8ULL     >::value );
  TTS_EXPECT( bool_<get_or_eval<float>  (values, or_else) == 1337.42f >::value );
}